...
**Privacy.** No tracking. No per-request logging. The only analytics are in-memory aggregate counters per tool (see [Analytics](#analytics)), which reset on restart. The client IP is used for the rate-limit decision only and is not stored.
...
Aggregate-only, in memory (since WO#360). For each tool the server keeps a running count of calls by outcome (`ok`, `not_found`, `error`) and the summed handler time, so `/stats` can report an average. That is all it keeps.

- No per-request records, written anywhere.
- No IP addresses, user-agents, client identifiers or geolocation.
- No request parameters (so no `context` values or verse refs) and no response payloads.
- Nothing is persisted to disk; the counters reset when the process restarts.

Releases before WO#360 (1.2.0 and earlier on npm) wrote a per-request JSONL log with country, region, client and user-agent. That logging is removed; the text that described it is gone from this README.
